...
namespace MWWorld
{
...
#ifdef __vita__
        // Ref -> sole containing cell (single-cell ids only). Must run before
        // the count pass below moves refIDs entries out. Last occurrence per
        // RefNum wins, mirroring the forEachUnique(rbegin) count semantics.
        {
    ...
    void ESMStore::vitaSeedRefCounts(std::unordered_map<ESM::RefId, int>&& counts, const std::vector<ESM::RefId>& keyIds)
    {
        ...
        // Non-empty mRefCount makes countAllCellRefsAndMarkKeys early-return.
    }
